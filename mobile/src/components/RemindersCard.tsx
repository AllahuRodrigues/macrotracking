...
import { View, Switch, Pressable, Alert, ActivityIndicator } from "react-native";
  ...
  return (
    <Card>
      <Row style={{ gap: 8, marginBottom: 4 }}>
        ...
        <AppText bold size={17}>Phone reminders</AppText>
      </Row>
      <AppText muted size={12} style={{ marginBottom: 12 }}>
        Pop-up alerts on this iPhone — workout, shakes, water, logging. {count > 0 ? `${count} scheduled.` : ""}
      </AppText>

      {!prefs.enabled ? (
        <Button label={busy ? "Setting up…" : "Turn on all reminders"} onPress={enableAll} loading={busy} />
      ) : (
        <>
          ...
          <ToggleRow label="Protein target check (8 PM)" value={prefs.proteinCheck} onChange={toggle("proteinCheck")} />
          ...
          {prefs.workout && (
            <Row style={{ justifyContent: "space-between", marginBottom: 8, marginLeft: 4 }}>
              <AppText muted size={13}>Workout time</AppText>
              <Row style={{ gap: 12 }}>
